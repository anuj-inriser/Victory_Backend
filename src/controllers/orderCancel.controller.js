...
const cancelOrder = async (req, res, next) => {
  const pool = getPool();
  try {
    const { variety, internal_order_id } = req.body;
    ...
    const remainingData = await pool.query(
      "select quantity,price,duration,producttype,tradingsymbol,symboltoken,exchange,ordertype,broker,transactiontype,squareoff,stoploss,orderid from transactions.orders_transactions where internal_order_id=$1",
      [internal_order_id]
    );
    ...
    const jsonData = {
      ...payload,
      internal_type: "CANCEL_ORDER",
      response_timestamp: responseTimeStamp,
      quantity: remainingData.rows[0].quantity,
      price: remainingData.rows[0].price,
      duration: remainingData.rows[0].duration,
      producttype: remainingData.rows[0].producttype,
      tradingsymbol: remainingData.rows[0].tradingsymbol,
      symboltoken: remainingData.rows[0].symboltoken,
      exchange: remainingData.rows[0].exchange,
      ordertype: remainingData.rows[0].ordertype,
    ...
    console.log("error in Cancelling order controller", error);
    next(error);
  }
};
