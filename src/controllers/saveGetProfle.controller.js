...
const saveGetProfile = async (req, res, next) => {
  const pool = getPool();
  try {
    ...
    const result = await pool.query(
      "Call save_get_profile_create($1,null,null,null)",
    ...
    console.log("error while fetching profile data", error.message);
    next(error);
  }
};

module.exports = { saveGetProfile };
