const getPool = require("../db/db.js");

const getAllPins = async (req, res, next) => {
  const pool = getPool();

  try {
    const result = await pool.query("SELECT * FROM pin_get_all()");

    return res.status(200).json({
      status: true,
      message: "Fetched all Pin records successfully",
      data: result.rows,
    });
  } catch (error) {
    console.error("Error in getAllPins:", error);
    next(error);
  }
};

const getPinById = async (req, res, next) => {
  const pool = getPool();
  const { id } = req.params;

  try {
    const result = await pool.query("SELECT * FROM pin_get_by_id($1)", [id]);

    if (result.rows.length > 0) {
      return res.status(200).json({
        status: true,
        message: "Pin record fetched successfully",
        data: result.rows[0],
      });
    } else {
      return res.status(404).json({
        status: false,
        message: "Pin record not found",
      });
    }
  } catch (error) {
    console.error("Error in getPinById:", error);
    next(error);
  }
};

const createPin = async (req, res, next) => {
  const pool = getPool();
  const jsonData = req.body;

  try {
    const result = await pool.query("CALL pin_create($1, NULL, NULL, NULL)", [
      JSON.stringify(jsonData),
    ]);

    const { out_status, out_message, out_result } = result.rows?.[0] || {};

    if (out_status) {
      return res.status(201).json({
        status: out_status,
        message: out_message,
        id: out_result,
      });
    } else {
      return res.status(400).json({
        status: out_status,
        message: out_message,
      });
    }
  } catch (error) {
    console.error("Error in createPin:", error);
    next(error);
  }
};

const updatePin = async (req, res, next) => {
  const pool = getPool();
  const jsonData = { ...req.body, id: Number(req.params.id) };

  try {
    const result = await pool.query("CALL pin_update($1, NULL, NULL, NULL)", [
      JSON.stringify(jsonData),
    ]);

    const { out_status, out_message, out_result } = result.rows?.[0] || {};

    if (out_status) {
      return res.status(200).json({
        status: out_status,
        message: out_message,
        id: out_result,
      });
    } else {
      return res.status(404).json({
        status: out_status,
        message: out_message,
      });
    }
  } catch (error) {
    console.error("Error in updatePin:", error);
    next(error);
  }
};

const deletePin = async (req, res, next) => {
  const pool = getPool();
  const jsonData = { id: parseInt(req.params.id, 10) };

  try {
    const result = await pool.query("CALL pin_delete($1, NULL, NULL, NULL)", [
      JSON.stringify(jsonData),
    ]);

    const { out_status, out_message, out_result } = result.rows?.[0] || {};

    if (out_status) {
      return res.status(200).json({
        status: out_status,
        message: out_message,
        id: out_result,
      });
    } else {
      return res.status(404).json({
        status: out_status,
        message: out_message,
      });
    }
  } catch (error) {
    console.error("Error in deletePin:", error);
    next(error);
  }
};

module.exports = {
  createPin,
  updatePin,
  deletePin,
  getAllPins,
  getPinById,
};
